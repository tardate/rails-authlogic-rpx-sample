...
* One of the most common mistakes made when trying to setup the sample is NOT SETTING the RPX_APP_NAME to the application you registered at rpxnow.com. If you do this, you will get an authorization failure.
* Make sure the application name is in lowercase. A mixed-case name can result in SSL certificate warning errors in some browsers (notably Android webkit)
...
1. Replace the authlogic_rpx config.gem with a load_paths specification in config/environment.rb:
	...
	config.load_paths += %W( c:\\experimental\\authlogic_rpx\\lib )
